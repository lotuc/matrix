(ns demo.x001-hello-world
  (:require
   ...
   [react]
   [tiltontec.matrix.api :refer-macros [mpar cF] :as mx]))
...
(defn Slider []
  (mxr/mkc ReactSlider
    {:className "horizontal-slider"
     ...
     ;; Notice the component will be reactive to the model properties from
     ;; itself. If you remove the counter from the component's model and fetch
     ;; the counter value directly
     ;; with (mx/mget (mx/fmu :counter-button) :counter), the value won't be
     ;; reactive.
...
(defn app []
  ...
  ;;
  ;; mx-props is optional, and it should be a *compile time map*. meaning
  ;; that it should be something like {:key <value>}.
  ;;
  ...
  (mxr/div {}
    {:name :root-app}
    ;; All the following arguments are children children are flattened and
    ;; nils removed, so this nested array is no problem.
    ...
    (mxr/button
      ;; --------------------------------------------------------------
      ;; React element props: these props are passed to createElement. The map
      ;; will be (deeply) converted to JS by clj->js. Note: We can pull
      ;; properties from 'me' (or elsewhere), but not use formulas here.
      {:disabled (mx/mget me :disabled?)
       ...
       :style {:color (mx/mget me :tx-color)
               ...
               :padding "1px 8px"}
       ;; Below: 'me' is bound to an atom holding an MX map/instance so we
       ;; use a swap! wrapper that adds reactive notification to modify the
       ;; counter. We could just `swap!`, but then no dataflow
       :onClick  #(mx/mswap! me :counter inc)}

      ;; ----------------------------------------------------------------------
      ;; This second group of props are for the Matrix "host" object. These
      ;; can be moderated by reactive Cells, or just be assigned literals.
      ;;
      {;; we give this button's mx model a name, we can find it by name and
       ;; use the attributes of this model later.
       :name :counter-button
       ;; cI is short for "input cell".
       ;;
       ;; We provide the starting value, then arbitrary imperative code can
       ;; change it. The starting value form is evaluated immediately.
       :counter   (mx/cI (+ 2 2))

       ;; cF is short for "formulaic cell".
       ;;
       ;; The formula will be first evaluated as soon as this Button instance
       ;; is added to the Matrix tree, then evaluated each time its
       ;; dependencies change.
       ;;
       ;; Notes:
       ...
       ;; TryThis[times-six,avg]: counting to 42 (to test the above) takes a
       ;; lot of clicking! Add a new computed ":times-six" property that is
       ;; six times the :counter property, and have :disabled? watch for
       ;; /that/ property to get to 42.
       }
    ...
    (mx/make ::js-window-location
      ...
      :listener (mx/cFonce
                  (let [cb #(let [u (js/URL. (.-url (.-destination %)))]
